...
# FILL IN THE TITLE AND THE SECTION BELOW 

project_name <- '' # the short name used for the files, e.g. gola
project_fullname <- '' # the full name to be used in the text, e.g. Gola Protected Area
country <- '' # country must correspond to a country in the naturalearthdata package. Check by running from the console:
start_year <- '' # as numeric
evaluation_year <- '' # as numeric
homedir <- ('C:/Users/Abby Williams/OneDrive - University of Cambridge/evaluations') # path to your evaluations folder
# 
```

```{r directory_structure, echo=FALSE,warning=FALSE, message=FALSE}

# set up necessary paths

script_path <- paste0(homedir,'/scripts')
supplier_path <- paste0(homedir,'/suppliers/',project_name)
data_path <- paste0(supplier_path,'/data')
output_path <- paste0(supplier_path,'/outputs')
shapefile <- read_sf(paste0(homedir,'/shapefiles/',project_name,'.geojson'))

path_list <- c(supplier_path,data_path,output_path)
for(i in path_list){
  if(!dir.exists(i))
    dir.create(i)
}
...
```{r sherwood_commands, include=FALSE}

# FILL IN YOUR CRSID AND RUN THIS CHUNK

crsid <- 'aew85'

paths <- c(
  "Upload shapefile: scp \"[PATH]/shapefiles/[PROJECT NAME].geojson\" sherwood:/maps/[YOUR CRSID]/projects",
  "Download additionality: scp sherwood:/maps/[YOUR CRSID]/tmf_pipe_out/[PROJECT NAME]/additionality.csv \"[PATH]/suppliers/[PROJECT NAME]/data\"",
  "Download carbon density: scp sherwood:/maps/[YOUR CRSID]/tmf_pipe_out/[PROJECT NAME]/carbon-density.csv \"[PATH]/suppliers/[PROJECT NAME]/data\"",
  "Download pairs: scp -r sherwood:/maps/[YOUR CRSID]/tmf_pipe_out/[PROJECT NAME]/pairs \"[PATH]/suppliers/[PROJECT NAME]/data\"",
  "Download stability (optional): scp sherwood:/maps/[YOUR CRSID]/tmf_pipe_out/[PROJECT NAME]/stopping.csv \"[PATH]/suppliers/[PROJECT NAME]/data\""
)

paths_replaced <- gsub("\\[YOUR CRSID\\]", crsid, paths)
paths_replaced <- gsub("\\[PATH\\]", homedir, paths_replaced)
paths_replaced <- gsub("\\[PROJECT NAME\\]", project_name, paths_replaced)

paths_result <- paste(paths_replaced, collapse = "\n")

# NOW RUN cat(paths_result) FROM THE CONSOLE TO PRINT THE PATHS
...
if(!file.exists(file.path(output_path,'tidy_data.csv'))){
  ...
  write.csv(data,file.path(output_path,'tidy_data.csv'),row.names=F)
  
} else {
  
  data <- read.csv(file.path(output_path,'tidy_data.csv'))
...
additionality <- read.csv(file.path(data_path,'additionality.csv'))
...
country_map <- ne_countries(country = country, returnclass = "sf")
...
if (file.exists(file.path(output_path,'luc_timeseries.csv'))) {
  result <- read.csv(file.path(output_path,'luc_timeseries.csv'))
} else {
  result <- luc_class1_uncertainty(data=data, t0=start_year, tend=evaluation_year)
  write.csv(result,file.path(output_path,'luc_timeseries.csv'),row.names=F)
}
...
carbon_density <- read.csv(file.path(data_path,'carbon-density.csv'))
