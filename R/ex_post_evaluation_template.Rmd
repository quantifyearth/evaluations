...
# Rscript -e "rmarkdown::render(input='~/evaluations/R/ex_post_evaluation_template.Rmd', output_file='~/evaluations/R/example_project.html', params=list(proj='example_project', t0=2010, eval_year=2022, input_dir='/maps/aew85/projects', output_dir='/maps/aew85/tmf_pipe_out', evaluations_dir='~/evaluations'))"
...
# IF RUNNING THIS NOTEBOOK MANUALLY, SET THE FOLLOWING PARAMETERS

project_name <- params$proj
start_year <- as.numeric(params$t0)
evaluation_year <- as.numeric(params$eval_year)
input_dir <- params$input_dir # path to directory in which the project shapefile is located
tmf_output_dir <- params$output_dir # path to tmf output directory
evaluations_dir <- params$evaluations_dir # path to evaluations directory
...
script_path <- paste0(evaluations_dir,'/scripts')
data_path <- paste0(tmf_output_dir,'/',project_name)

if (!is.null(params$shapefile_dir)) {
  shapefile_dir <- c(params$shapefile_dir)
} else { shapefile_dir <- paste0(input_dir,'/',project_name,'.geojson') }
shapefile <- read_sf(shapefile_dir)
