...
# mandatory args: proj, t0, input dir, output dir
# optional args: fullname, country, country_path, shapefile path, pairs_path, carbon density path. Use these if your file naming system deviates from that specified
...
`r if (!is.null(params$fullname)) { params$fullname } else { gsub('_', ' ', project_name) %>% stringr::str_to_title() }` is located in `r country_string`. The proposed area measures `r format(project_area_ha, big.mark = ",", scientific = FALSE, digits = 3)` hectares. Its proposed start date is `r start_year`.
...
More information about the 4C approach can be found below.
...
To estimate future deforestation pressure, we can look at past deforestation trends within our matched pixels. We can do this because our matching process is offset in time with respect to deforestation. This means that our project pixels are similar in terms of deforestation rate to that experienced by the matched pixels 10 years ago. Therefore, we can think of the matched pixels as being a historical representation of the project as it is today. By measuring deforestation in the matched pixels in the 10 years prior to the project start, we can forecast baseline deforestation into the future.

***Land cover changes over time***

In the below plots, we show the changes in various land classes - undisturbed forest, degraded forest, deforested and reforested land - over time. Note the offset in time between the project and the matched pixels. The vertical grey dashed line represents the start year of the project.
...
Focusing on the trend for undisturbed forest only:
...
df_rate_ha %>% rename('Rate (ha/year)' = 3)  
...
carbon_density_format

```


### Baseline rate of carbon emissions
